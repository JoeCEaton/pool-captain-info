...
- **Every player and logged match in the screenshots is fictional.** The names are invented
  ...
  own calculations so the figures are self-consistent. No real player's record appears
  anywhere in this repository.
- The two break screenshots are the exception: the review screen and the break log are
  genuine captures, because a measured break is not something that can be faked
  convincingly. They carry speeds and distances, not people.
- Everything else was captured from a release build on an Android emulator; the two
  break screenshots came from a phone.
